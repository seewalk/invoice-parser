...
export default function InvoiceDownloadButtons({ template }: InvoiceDownloadButtonsProps) {
  ...
  const generateAndDownloadPDF = async () => {
    ...
    try {
      ...
      if (result.success && result.pdfBase64 && result.fileName) {
        ...
        // Show upgrade prompt 2 seconds after download (non-aggressive timing)
        setTimeout(() => {
  ...
  const handlePDFDownload = async () => {
    ...
    if (isLeadCaptureRequired) {
      console.log('[Template Download] Lead capture required, showing modal');
    ...
    console.log('[Template Download] Lead already captured, proceeding with download');
  ...
  const handleLeadCaptured = async (data: any) => {
    console.log('[Template Download] Lead captured, proceeding with PDF generation');
